...
suite per lecture (L01 to L05 and L08 to L10), L06's conv demo printing its results next to the
numbers you worked out by hand in [L06's appendix B](../lectures/L06/appendix/b_exercises.md), and
the self-assessment questions in [L10's appendix C](../lectures/L10/appendix/c_exercises.md). Every
one of those checks a component you built or a number you computed beside it.
...
Both cover the whole course, L01 to L10, and share no question. Either can be used alone; use both
as a main sitting and a resit, or in alternate years.

They do **not** weight the course identically, and that is deliberate rather than accidental. Paper
A spends more on the convolutional layer itself (L06 is worth roughly three times as much there),
Paper B spends more on the finished network and its constraints (L10 is worth twice as much). Two
topics appear on one paper only: L02's precision and early stopping is examined on **B alone**
(Q2c, Q2d), and the fixed-point and real-time material from L10's appendix D is likewise **B alone**
(Q9b, Q9c). Paper A is also the only one that asks a candidate to write a method from scratch. If
you are using the papers as a sitting and a resit, that asymmetry is worth knowing; if you need the
two to examine exactly the same ground, sit both.
...
| 9        | The finished network, and the target it has to run on | L10           | 8       |
|          |                                                       |               | **100** |
...
* **The course's sign convention.** The deviation is $\delta = y_{ref} - y_p$ and every parameter is
  *increased* by its adjustment, so every update reads `parameter += gradient * learningRate` with
  no minus sign. A candidate who flips the sign consistently and says so is not penalized twice.
...
* **Only the values nobody could be expected to carry are supplied**: a handful of `tanh()` values,
  $2^{16}$ on Paper B, and that a KB is 1024 bytes. Parameter counts, output shapes and padding
  sizes are derived, because deriving them is the examinable skill.
* **Code questions refer to the `Dense`/`Shallow` API specified in L03 to L05**, the one the
  candidate builds, not to the refactored API in `cnn_demo`. The two differ: `cnn_demo`'s `Dense`
  reverses the constructor arguments, renames the accessors, collapses the two `backpropagate()`
  overloads into one, and carries a seventh member variable. Answers written against either are
  accepted, but the model answers follow L03 to L05.
...
* **Method carries the marks.** A correct update rule with an arithmetic slip in it is worth more
  ...
  earlier answers. Follow through an error rather than penalizing it twice.

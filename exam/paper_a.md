...
**(d)** The L03 stub exposes two methods that are not part of `dense_layer::Interface`:
...
Give the 4 x 4 gradient matrix the layer passes back. (2 marks)

**(c)** A student writes `double maxValue{0.0};` before the search, leaves `maxRow` and `maxCol`
default-initialised, and ends the block with

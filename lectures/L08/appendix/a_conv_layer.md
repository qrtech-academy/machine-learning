...
Add two more accessors, which are **not** part of `conv_layer::Interface`. The tests call them on a
`const Conv`, so both must be `const`:
* **`kernel()`**: `[[nodiscard]] const Matrix2d& kernel() const noexcept;`, returns `myKernel`.
* **`bias()`**: `[[nodiscard]] double bias() const noexcept;`, returns `myBias`.
...
2. In `source/ml/factory/factory.cpp`, find `Factory::convLayer()`: it currently returns a
   `conv_layer::ConvStub` (marked with a `//! @todo` comment). Replace that with your new class:
   ```cpp

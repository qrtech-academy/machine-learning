...
**Optimization:** works identically to dense layers; each kernel weight and the bias value are
adjusted along their gradient, scaled by the learning rate. The gradient is added rather than
subtracted, since the deviation is computed as `y_ref - y_p` and so already points in the
direction that reduces the error. The only difference is that

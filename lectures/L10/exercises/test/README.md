...
Unchanged from L09; see [that suite's README](../../../L09/exercises/test/README.md).
...
| `FeedforwardMatchesHandTrainedExample`     | L06 appendix B's flattening        |
...
is that the two directions agree. A *mismatch* is what does the damage, because then gradients land
on the wrong pixels and the conv layer learns from noise. This test flattens a matrix and feeds the
result straight back in, so it fails if the two directions ever disagree. The order itself is still
pinned down separately: appendix A section 5 mandates row-major, and `FeedforwardIsRowMajor` and
`BackpropagateIsRowMajor` check it, so a self-consistent column-major layer passes this test and
fails those two.
...
`TrainRejectsInvalidLearningRate` includes `1.0`, which is rejected because `Cnn::train()` and every
layer share the same `ml::checkLearningRate()`, and that compares strictly at both ends. The bound
is worth checking whenever you touch `train()`: a looser check there would let `1.0` through to a
full feedforward and backpropagate before a layer turned it down.
...
**The network's own learning rate check.** `Cnn::train()` and every layer share one check,
`ml::checkLearningRate()`, so all of them accept exactly `(0.0, 1.0)`. Removing the check from
`train()` changes nothing observable: the conv layer turns the same rates down a moment later, so
`train()` still returns `false`, though only after a feedforward and backpropagate have already
run. `TrainRejectsInvalidLearningRate` pins the behaviour down, not which layer enforces it.

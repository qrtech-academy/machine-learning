# Appendix A - Course Review

## Overview
Every layer built in this course — from L01's linear regression up through L06-L09's CNN layers —
...
| `Conv` | L06-L07 | one shared kernel + bias | same as `Dense`, but local (small window) and shared (same kernel at every position) | gradients accumulated across every position the kernel visited, then applied once | `kernel,bias += gradient * LR` |
| `MaxPool` | L06, L08 | none | passes the largest value in each block forward | routes the incoming gradient only to the position that held the max value; everywhere else gets `0` | no-op — nothing to update |
| `Flatten` | L06, L09 | none | reshapes 2D → 1D | reshapes 1D → 2D (the exact inverse) | doesn't exist in the interface — there's nothing to optimize |
...
## 2. The Full Pipeline (`cnn_demo`, from L06)
`ml::cnn::Cnn` (see [`cnn_demo`](../../L06/cnn_demo)) wires four of these layer types together into
one working classifier:

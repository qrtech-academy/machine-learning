...
The `Dense`, `Conv`, `MaxPool`, and `Flatten` implementations built during this course, and the
standalone `ml::ConvLayer`, `ml::MaxPoolLayer` and `ml::FlattenLayer` structs from L06/L07, all use
`std::vector`, which allocates on the heap. On many embedded targets, heap

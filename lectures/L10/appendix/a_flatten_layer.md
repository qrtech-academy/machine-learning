...
digits 0-3 and checks the right output node wins for each; the closest thing this
course has to a final exam for your code.

Two things worth knowing before you start:

* The one flatten layer test to read is `BackpropagateUndoesFeedforward`. What really has to hold
  is that `backpropagate()` undoes exactly what `feedforward()` did; if the two disagree, gradients
  land on the wrong pixels and the conv layer quietly learns from noise, so the network still runs,
  it just never gets good. Note though that section 5 above mandates **row-major** order, and
  `FeedforwardIsRowMajor` and `BackpropagateIsRowMajor` check it, so a self-consistent column-major
  implementation still fails those two.

...
8. Most layer types in this course implement the same three methods: `feedforward()`,
   `backpropagate()`, and `optimize()`. What does `optimize()` do for a max pooling layer, which
   has no trainable parameters? And why does the flatten layer's interface not declare
   `optimize()` at all, rather than declaring it and leaving it empty?

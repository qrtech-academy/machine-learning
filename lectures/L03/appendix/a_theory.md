...
#### 1. Feedforward
* The nodes in the input layer form the inputs x to every node in the hidden layer.
...
$$s = b + \sum_{i=0}^{n-1} w_i * x_i = b + w_0 * x_0 + w_1 * x_1 \ldots + w_{n-1} * x_{n-1},$$
...
$$y_p = \sigma(s) = \sigma\!\left(b + \sum_{i=0}^{n-1} w_i * x_i\right)$$
...
$$\delta = \sum_{i=0}^{n-1} [\Delta e_i * w_i]$$
...
| Leaky ReLU | $(-\infty, \infty)$ | Hidden layers, alternative to ReLU | Extra hyperparameter (α) to choose |
...
$$y = \sigma(s) = \sigma\!\left(b + \sum_{i=0}^{n-1} w_i * x_i\right)$$
...
* As a rule of thumb, α = 0.01 can be used.
* Leaky ReLU counteracts nodes "dying" (getting stuck at zero), which can otherwise cause the model to train poorly.
...
the same number, not a different argument. See **L05** for why the layer keeps $s$ around
alongside $y$.
...
$$\sigma'(s) = y * (1 - y), \quad \text{where } y = \sigma(s)$$
...
#### 4. Softmax
Softmax is a common activation function in the output layer, where the sum of all output probabilities equals one. The output y for a node in a layer of n nodes:

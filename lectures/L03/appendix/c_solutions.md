...
Full solutions to the three hand-training examples in [appendix B](./b_exercises.md). Each one
is worked through in four steps: feedforward, backpropagation, optimization, and a verification
that feeds the same input forward again with the updated parameters.

Try each example yourself before reading its solution.

---

## Example 1
The odd-parity network, trained on $X_1X_2 = 01$, $Y = 1$.
See [appendix B](./b_exercises.md#hand-training-a-neural-network---example-1) for the full statement.
...
$$e_1 = \delta_1 * \sigma'(s_1) = -0.076 * 1 = -0.076$$

$$e_2 = \delta_2 * \sigma'(s_2) = -0.684 * 1 = -0.684$$
...
## Example 2
The network that inverts its two inputs, trained on $X_1X_2 = 10$, $Y_1Y_2 = 01$.
See [appendix B](./b_exercises.md#hand-training-a-neural-network---example-2) for the full statement.
...
## Example 3
The odd-parity network after one epoch, trained on $X_1X_2 = 11$, $Y = 0$.
See [appendix B](./b_exercises.md#hand-training-a-neural-network---example-3) for the full statement.
...
$$e_1 = \delta_1 * \sigma'(s_1) = -0.35 * 1 = -0.35$$

$$e_2 = \delta_2 * \sigma'(s_2) = -0.4 * 0 = 0$$

Note that $s_2 = -0.5$, which means $\sigma'(s_2) = 0$ since the derivative of ReLU is zero for weighted sums $\leq 0$. Node 2 therefore doesn't contribute to the error and won't be updated in this step.

...
**Note!** Nodes in neural networks are somewhat more complex than that: the sum is passed through an *activation function*, which for example clamps negative sums to zero (ReLU). The bias plays the role a threshold would, shifting where that function's behaviour changes, but the function is applied to every sum unconditionally rather than gating the node on or off. Neural networks are covered later in the course.
...
* A special case for `x = 0`, where the bias is set directly to the reference value (`bias = output`). Worth knowing that this is a shortcut, not a better learning rule: it overwrites a trained parameter from a single sample, so with noisy data, or with more than one training set at `x = 0`, the final bias is just whichever of them was visited last. The ordinary update rule already handles `x = 0` correctly, since `k` has no effect on a prediction made with an input of zero.

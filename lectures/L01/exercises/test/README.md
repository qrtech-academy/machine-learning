...
| Suite | Covers |
|---|---|
| `LinRegFixed` | Construction, argument validation of `train()`, and the parameter updates made by `optimize()`, one training step at a time, with every expected value computed by hand. |
| `LinRegComponent` | Full training runs used through `ml::lin_reg::Interface`, checking that the trained model reproduces the line hidden in the training data, including for inputs it was never trained on. |

Two details worth knowing about while reading the tests:

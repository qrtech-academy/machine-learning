...
Neither branch enforces the limits itself. Both only move the rate by the step, and the limits are
applied once afterwards:

```cpp
learningRate = std::clamp(learningRate, minLearningRate, maxLearningRate);
```

`std::clamp()` comes from `<algorithm>`, which `adaptive.cpp` already includes for `std::min()`,
and it returns the nearer bound whenever the value falls outside the range, i.e. exactly what a
pair of hand-written `if` statements would do. One call covers both ends, so each branch is left
saying only what it's actually about: raise the rate, or lower it. Mind the argument order, value
first, then the low bound and the high one; handing it the bounds the wrong way round is undefined
behaviour rather than an error the compiler reports.

That one clamp is what keeps the rule from destroying the model. Its upper bound of `0.25` stays
below the rate at which this kind of training data starts to oscillate rather than converge, which
**L01**'s tuning table puts at around `0.29`. Its lower bound keeps the rate positive, so a model
that keeps overshooting slows down instead of stopping dead.

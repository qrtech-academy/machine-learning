...
If the model is always trained in the same order, it risks becoming too fitted to that particular sequence; it learns the pattern in the ordering rather than in the data. By shuffling the training order every epoch, the model is exposed to the data in varying order, which produces more robust training and stops the parameters from settling into a rhythm dictated by the ordering. For a linear model there are no local minima to get stuck in, since its squared-error surface has exactly one minimum; that benefit arrives with the deeper, non-linear networks from **L03** onward, where the same shuffling helps the parameters escape poor regions of the loss surface.
...
Note that drawing `r` from the whole range on every pass makes some orderings more likely than
others, so this shuffle isn't uniform. It's plenty for varying the training order, which is all
it's used for here. Drawing from the remaining range instead, `i + std::rand() % (order.size() - i)`,
gives a uniform shuffle for the same effort; that's the Fisher-Yates algorithm, and `std::shuffle()`
from `<algorithm>` is what you'd normally reach for outside a course like this.
...
**A note on the name.** This is not *precision* in the sense the wider machine learning literature uses it, where precision counts how many of the positive predictions were right and only applies to classification. What's computed here is a fit score for a regression model, and it has two properties worth knowing: it's scale-dependent, since an absolute error of `0.1` means something very different for outputs around `1` than for outputs around `1000`, and it can go negative, since nothing stops the mean absolute error from exceeding `1.0`. Reading `0.999999` as "99.9999 % correct" is therefore a convenient shorthand rather than a percentage of anything.

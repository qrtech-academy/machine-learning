...
* **Didn't improve at all:** lower the learning rate by the step. The model is overshooting, so take smaller steps.

Neither branch should enforce the limits itself. Once the rate has been adjusted, keep it inside
them with a single call to `std::clamp()` (from `<algorithm>`, already included for `std::min()`):
`learningRate = std::clamp(learningRate, minLearningRate, maxLearningRate)`. It returns the nearer
bound whenever the value falls outside the range, so one call covers both ends and each branch is
left saying only what it's about.
...
training data starts to oscillate instead of converging, and its minimum keeps the rate positive,

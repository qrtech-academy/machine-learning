...
[appendix A](../../appendix/a_max_pool_layer.md), written with the
...
back to exactly the four positions L06's appendix B sends them to. If those numbers look familiar, they
...
### `ml::conv_layer::Conv`
Unchanged from L08; see [that suite's README](../../../L08/exercises/test/README.md) for
the breakdown.
...
| `FeedforwardMatchesHandTrainedExample`         | L06 appendix B's pooling, exactly    |
...
| `BackpropagateMatchesHandTrainedExample`       | L06 appendix B's routing, exactly    |
...
| `BackpropagateResetsGradientsBetweenCalls`     | nothing left over from the last call |
...
`BackpropagateResetsGradientsBetweenCalls` feeds two inputs whose maxima sit in different
positions, backpropagating after each, and checks that the second call leaves nothing where the
first input's maxima were. Backpropagating the same input twice would miss the likeliest way to get
this wrong: a layer that writes each gradient with `=` and never clears the rest writes the same
cells both times, so it only goes wrong when the positions change, which in training they do from
one example to the next. Nothing else in the suite catches it, and the network still trains well
enough most of the time for the component tests in L10 to pass.

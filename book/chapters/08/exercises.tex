...
% suite's README, lectures/L08/exercises/test/README.md. The solution to the code is in
% the repository.
...
\textbf{How to check your work.} The test suite in \file{lectures/L08/exercises/test}
...
\begin{itemize}
  ...
  \item Keep \code{[[nodiscard]]} on every override whose interface method has it. Attributes
    aren't inherited, so the override needs its own (see Chapter~\ref{c3:ch:nn}).
\end{itemize}
...
\label{c8:ex:10}
A test suite for \code{Conv} is available in \file{lectures/L08/exercises/test}. It's the
...
The test suite's README, \file{lectures/L08/exercises/test/README.md}, has more

...
% examples, which lectures/L03/appendix/c_solutions.md holds, are in Appendix A; the solution to the
...
codebase in this lecture's exercises directory, \file{lectures/L03/exercises} in the
...
\file{lectures/L03/exercises/test}, which Exercise~\ref{c3:ex:7} runs. It tests the
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Should be marked \code{override}, retaining the interface's \code{[[nodiscard]]},
      \code{const}, and \code{noexcept}.
    \item Attributes aren't inherited, so the override needs its own \code{[[nodiscard]]}. Without
      it, a discarded call such as \code{stub.nodeCount();} compiles silently whenever it's made on
      a \code{Stub} rather than through an \code{Interface&}.
  \end{itemize}
...
\label{c3:ex:7}
A test suite for the stub is available in \file{lectures/L03/exercises/test}. It's already
...
The test suite's README, \file{lectures/L03/exercises/test/README.md}, has more

...
Call the network's private \code{feedforward(myTrainInput[x])}. This performs feedforward through
both layers and returns \code{false} if either layer rejected its input. \code{predict()} performs
the same pass and then returns the output layer's output, but training has no use for that output.
...
\noindent The learning rate is therefore never supplied by the caller. It starts at \code{0.01} and
is revised from one evaluation to the next, much as in \code{ml::lin_reg::Adaptive} from

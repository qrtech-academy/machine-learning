...
\file{lectures/L04/exercises} in the companion repository, and build it with your own
...
\file{lectures/L04/exercises}, then extend the structure as follows:
...
\end{itemize}
Both \code{predict()} and \code{train()} feed an input through the whole network, so that goes in a
private method of its own:
\begin{itemize}
...
output layer's output. It isn't marked \code{[[nodiscard]]} for the same reason: \code{predict()}
has no way to pass the result on, so it ignores it.
...
\begin{itemize}
  ...
  \end{enumerate}
\end{itemize}
Each of the five calls above returns \code{bool}: \code{feedforward()} and the four layer calls
(see Chapter~\ref{c3:ch:nn}). Return \code{false} as soon as any of them fails: a dimension mismatch means the network is wired wrong,

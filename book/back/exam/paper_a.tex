...
\qpart{c} Each of the five calls in the inner loop returns \code{bool} (the network's own
\code{feedforward()} and the four layer calls), and every one of those return values is discarded.
State what the five calls report through that value, name the failure it was meant to
